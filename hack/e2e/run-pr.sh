...
chainsaw_suites="${E2E_CHAINSAW_SUITES:-cluster,cluster-scale,cluster-rolling-update,failover,failover-switch,failover-scale,failover-rolling-update}"
...
for raw_suite in "${suites[@]}"; do
  ...
  case "$suite" in
    cluster|cluster-scale|cluster-rolling-update|cluster-external|failover|failover-switch|failover-scale|failover-rolling-update|failover-external)
...
if [ "${#chainsaw_suite_dirs[@]}" -eq 0 ]; then
  echo "E2E_CHAINSAW_SUITES must include at least one suite (cluster, cluster-scale, cluster-rolling-update, cluster-external, failover, failover-switch, failover-scale, failover-rolling-update, failover-external)" >&2
  exit 1
fi
