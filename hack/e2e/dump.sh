...
kubectl -n redis-operator-system get all -o wide >"${artifact_dir}/operator-all.txt" 2>&1 || true
kubectl -n redis-operator-system logs deployment/redis-operator-controller-manager -c manager --tail=-1 >"${artifact_dir}/operator-manager.log" 2>&1 || true
